...
`;const R=`/api/generate?domain=${encodeURIComponent(a)}&target=${encodeURIComponent(k)}&level=${encodeURIComponent(F)}&language=${encodeURIComponent(I)}&model=${encodeURIComponent(w)}${O?"&skip_cache=true":""}`,S=new t.EventSource(R);S.addEventListener("log",N=>{const{message:ie}=JSON.parse(N.data);y.textContent+=ie+`
`,y.scrollTop=y.scrollHeight}),S.addEventListener("done",()=>{S.close(),y.textContent+=`
✓ Done — reloading…`,setTimeout(()=>t.parent.location.reload(),1200)}),S.addEventListener("gen_error",N=>{S.close(),y.textContent+=`
✗ ${JSON.parse(N.data).message}`,u.disabled=!1,u.textContent="Retry",u.style.background="#dc2626"}),S.onerror=()=>{S.readyState!==t.EventSource.CLOSED&&(S.close(),y.textContent+=`
✗ API not reachable.
